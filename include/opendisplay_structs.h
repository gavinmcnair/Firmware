 ...
 *  transfer into a persistent on-device slot (flash-backed; survives deep
 *  sleep and power loss) instead of the live panel; the base
 *  PipeStartRequest.total_size field carries the COMPRESSED byte total
 *  being stored (must fit that board's OD_SLOT_SIZE_BYTES), not a
 *  decompressed size." */
struct PipeSlotExt {
    uint8_t  slot_id;           /**< @min 0 @max 99 @doc "target slot index; NACKed OD_ERR_PIPE_START_SLOT_INVALID if >= this board's runtime slot capacity." */
    uint8_t  reserved;          /**< @reserved @doc "must be 0." */
